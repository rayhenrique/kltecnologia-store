<x-admin-layout title="Novo Artigo do Blog">
    <div class="space-y-6">
        {{-- Breadcrumbs & Header --}}
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1">
                    ...
                    <span class="text-teal-700 font-bold">Novo Artigo</span>
                </nav>
                <h1 class="font-display text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">
                    Escrever Novo Artigo
                </h1>
                <p class="text-xs sm:text-sm text-slate-600 mt-1">
                    Compartilhe tutoriais, novidades e artigos educativos para os clientes da loja.
                </p>
            </div>

            <a 
                href="{{ route('admin.posts.index') }}" 
                class="btn-secondary text-xs !min-h-10 !px-4 flex items-center gap-2 self-start sm:self-auto shadow-2xs text-slate-700 font-semibold"
            >
