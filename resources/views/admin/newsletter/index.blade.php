    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                {{-- Botão Exportar CSV --}}
                <a 
                    href="{{ route('admin.newsletter.export', ['status' => request('status'), 'q' => request('q')]) }}" 
                    class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-300 px-3.5 py-2 text-xs font-bold text-slate-700 shadow-2xs transition cursor-pointer"
                ...
                <a 
                    ...
                    title="Cadastrar novo inscrito manualmente"
                >
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    <span>Novo Inscrito</span>
                </a>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($subscribers as $subscriber)
                            <tr class="hover:bg-slate-50/70 transition group">
                                ...
                                <td class="py-3.5 px-4 sm:px-6 font-medium text-slate-900">
                                    <div class="flex items-center gap-2">
                                        <a 
                                            ...
                                            class="font-mono text-xs sm:text-sm font-semibold text-slate-800 hover:text-teal-600 transition"
                                ...
                                {{-- Status com Toggle Rápido --}}
                                <td class="py-3.5 px-4">
                                    <form action="{{ route('admin.newsletter.toggle-status', $subscriber) }}" method="POST" class="inline">
                                        @csrf
                                        <button 
                                            type="submit" 
                                            class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold transition cursor-pointer {{ $subscriber->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 border border-slate-200 hover:bg-slate-200' }}"
                                            title="{{ $subscriber->is_active ? 'Clique para desativar este e-mail' : 'Clique para reativar este e-mail' }}"
                                        >
                                            <span class="h-1.5 w-1.5 rounded-full {{ $subscriber->is_active ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400' }}"></span>
                                            <span>{{ $subscriber->is_active ? 'Ativo' : 'Inativo' }}</span>
                                        </button>
                                    </form>
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <a 
                                            href="{{ route('admin.newsletter.show', $subscriber) }}" 
                                            class="rounded-lg p-1.5 text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition cursor-pointer"
                                            title="Ver Detalhes do Inscrito"
                                        >
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <a 
                                            ...
                                            class="rounded-lg p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition cursor-pointer"
