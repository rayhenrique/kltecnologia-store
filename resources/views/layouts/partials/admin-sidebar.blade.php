    ...
    <div class="w-full">
        ...
        <nav class="mt-2 space-y-1">
            ...
            <a 
                ...
                :title="(!{{ $isMobile ? 'true' : 'false' }} && sidebarCollapsed) ? 'Cupons ({{ \App\Models\Coupon::count() }})' : ''"
                class="relative flex items-center rounded-xl text-xs font-semibold transition group {{ request()->routeIs('admin.coupons.*') ? 'bg-teal-600 text-white shadow-sm shadow-teal-600/30 font-bold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}"
                :class="(!{{ $isMobile ? 'true' : 'false' }} && sidebarCollapsed) ? 'justify-center h-11 w-11 mx-auto p-0' : 'justify-between px-3 py-2 w-full'"
            >
                <div class="flex items-center gap-3 min-w-0">
                    <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.coupons.*') ? 'text-white' : 'text-slate-400 group-hover:text-teal-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
                    </svg>
                    <span x-show="{{ $isMobile ? 'true' : '!sidebarCollapsed' }}" x-cloak class="truncate whitespace-nowrap">Cupons</span>
                </div>
                <span x-show="{{ $isMobile ? 'true' : '!sidebarCollapsed' }}" x-cloak class="rounded-md bg-slate-900 px-1.5 py-0.5 text-[10px] font-mono text-slate-400 border border-slate-800 shrink-0">
